...
def create_app() -> Any:
	...
	async def execute_multi_task(orchestrator_id: str, request: MultiTaskRequest):
		...
		try:
			...
			supervisor_model = request.supervisor_model or "moonshot/kimi-k2-0711-preview"
			...
			result = await run_multi_agent_parallel(
				...
				max_cost_per_agent=(request.max_cost or 10.0) / max(1, len(request.workers) or 1),
	...
	async def get_orchestrator_status(orchestrator_id: str):
		...
		return {"orchestrator_id": orchestrator_id, "workers": len(orchestrators[orchestrator_id]["workers"])}

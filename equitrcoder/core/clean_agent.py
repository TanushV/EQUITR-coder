...
class CleanAgent:
    ...
    def __init__(
        ...
        self.audit_provider = LiteLLMProvider(model=audit_model) if audit_model != model else self.provider
    ...
    async def run(self, task_description: str, session_id: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    async def _execution_loop(self) -> Dict[str, Any]:
        ...
        while iteration < max_iterations:
            ...
            if self.max_cost and self.current_cost >= self.max_cost:
                return {"success": False, "reason": "Cost limit exceeded", "final_message": "Cost limit reached"}
            
            if self.on_iteration_callback:
                self.on_iteration_callback(iteration, {
                    "cost": self.current_cost,
                ...
                if response.tool_calls:
                    ...
                    for tool_call in response.tool_calls:
                        ...
                        if tool_name in self.tools:
                            ...
                            result_content = str(tool_result.data if tool_result.success else tool_result.error)
                    ...
                    if tool_results:
                        ...
                        self.add_message('user', results_message, {'system_generated': True})
                    ...
                    completion_indicators = [
                        "all todos completed", "all tasks finished", "work completed",
                        "all done", "finished successfully", "task complete"
                    ]
                    ...
                    self.add_message('user', warning_message, {'system_generated': True, 'warning': True})
                    continue
...
Be thorough but fair in your assessment."""
            ...
            audit_schemas = [self.tools[tool].get_json_schema() for tool in available_audit_tools]

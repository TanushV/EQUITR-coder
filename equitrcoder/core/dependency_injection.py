...
class DependencyInjectionContainer:
    ...
    def _resolve_service(self, service_type: Type[T]) -> T:
        ...
        if (
            ...
            and service_type in self._scoped_instances
        ):
    ...
    def _analyze_dependencies(self, implementation_type: Type) -> List[Type]:
        ...
        try:
                    ...
                    if isinstance(param.annotation, str):
                        ...
                        try:
                            ...
                            if module and hasattr(module, param.annotation):
                                ...
                                logger.warning(
                                    f"Cannot resolve forward reference '{param.annotation}' "
                                    f"in {implementation_type.__name__}"
                                )
                        except Exception:
                            logger.warning(
                                f"Failed to resolve forward reference '{param.annotation}' "
                                f"in {implementation_type.__name__}"
                            )
                    else:
            ...
            logger.warning(
                f"Failed to analyze dependencies for {implementation_type.__name__}: {e}"
            )
            return []
        ...
        def wrapper(*args, **kwargs):
            ...
            for param_name, param in signature.parameters.items():
                if (
                    ...
                    try:
                        injected_kwargs[param_name] = container.resolve(
                            param.annotation
                        )
                    except ServiceNotRegisteredError:
...
def register_singleton(
    service_type: Type[T], implementation_type: Optional[Type[T]] = None
) -> DependencyInjectionContainer:

...
class CleanOrchestrator:
    ...
    async def create_docs(
        ...
        try:
            ...
            design_content = await self._create_design(
                task_description, requirements_content
            )
            ...
            if num_agents > 1:
                ...
                todos_content = await self._create_todos(
                    task_description, requirements_content, design_content
                )
                ...
                todos_result = {"todos_path": str(todos_path), "agent_todo_files": []}
...
Be technical and specific. Use markdown format."""
    ...
    async def _create_todos(
        self, task_description: str, requirements: str, design: str
    ) -> str:
...
- [ ] Task 4"""
        ...
        agent_todo_files = await self._split_todos_for_agents(
            todos_content, num_agents, docs_dir
        )

        return {"todos_path": str(todos_path), "agent_todo_files": agent_todo_files}

    async def _split_todos_for_agents(
        self, todos_content: str, num_agents: int, docs_dir: Path
    ) -> List[str]:
                ...
                agent_categories = [
                    {
                        "name": "General Tasks",
                        "todos": [
                            "- [ ] Coordinate with other agents and integrate work"
                        ],
                    }
                ]
...
"""
            ...
            print(
                f"📋 Agent {agent_idx + 1} assigned: {[cat['name'] for cat in agent_categories]}"
            )

            # Setup todo system for this agent
            agent_todo_file = f".EQUITR_todos_agent_{agent_idx + 1}.json"
            await self._setup_todo_system_for_agent(
                agent_categories, agent_todo_file, agent_idx + 1
            )
    ...
    async def _setup_todo_system_for_agent(
        self, categories: List[Dict], todo_file: str, agent_id: int
    ):
    ...
    def get_context_for_agent(
        self, docs_result: Dict[str, Any], agent_id: Optional[int] = None
    ) -> Dict[str, Any]:

...
class CleanOrchestrator:
    ...
    async def create_docs(
        ...
        requirements_content, req_cost = await asyncio.wait_for(self._create_requirements(task_description, repo_context), timeout=req_timeout)
        ...
        design_content, des_cost = await asyncio.wait_for(self._create_design(task_description, requirements_content, repo_context), timeout=des_timeout)
    ...
    async def _create_requirements(self, task_description: str, repo_context: str) -> tuple[str, float]:
        ...
        # Strict: no fallbacks
        response = await asyncio.wait_for(self.provider.chat(messages=messages), timeout=300)
        return response.content, float(getattr(response, "cost", 0.0) or 0.0)
    
    async def _create_design(self, task_description: str, requirements: str, repo_context: str) -> tuple[str, float]:
    ...
    async def _setup_todo_system(self, task_description: str, requirements: str, design: str, task_name: str, todo_file_path: Path, team: Optional[List[str]] = None, repo_context: Optional[str] = None, is_research: bool = False):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                response = await asyncio.wait_for(self.provider.chat(messages=messages), timeout=180)
            ...
            try:
                ...
                if attempt == max_retries:
                    raise ValueError(f"Failed to get valid task groups after retries: {e}")
        ...
        for group_data in task_groups_data + [{'group_id': 'experiment_execution', 'specialization': 'experiment_runner'}]:
            ...
            try:
                ...
                for attempt in range(1, max_retries + 1):
                    try:
                        ...
                        raise TimeoutError(f"Todo generation timed out for {group_data['group_id']}")
                    except Exception as e:
                        raise e
                    try:
                        ...
                        if attempt == max_retries:
                            raise ValueError(f"Failed to get valid todos for group {group_data['group_id']}: {e}")
                ...
                print(f"    ✅ Added {len(todos_data)} lightweight todos to {group_data['group_id']}")
            except Exception as e:
        ...
        try:
            ...
            if exp_group and not exp_group.todos:
                manager.add_todo_to_group('experiment_execution', 'Create and run experiment runner script; execute experiments and log results')

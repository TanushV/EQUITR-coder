...
class FileCache(ICache[Any]):
    ...
    def __init__(
        ...
        logger.info(
            f"FileCache initialized: max_size={max_size}, max_memory={max_memory_mb}MB, ttl={default_ttl_seconds}s"
        )
    ...
    def invalidate_pattern(self, pattern: str) -> int:
        ...
        with self._lock:
            ...
            self._update_cache_stats()
            logger.info(
                f"Invalidated {len(files_to_remove)} files matching pattern: {pattern}"
            )
    ...
    async def set(self, key: str, value: Any, ttl: Optional[int] = None) -> bool:
        ...
        logger.warning(
            "FileCache.set() not supported - files are cached automatically when read"
        )
        return False
    ...
    def _is_cache_valid(self, entry: CacheEntry, file_path: str, ttl: int) -> bool:
        ...
        try:
            ...
            if (
                current_mtime != entry.modification_time
                or current_size != entry.file_size
            ):
                return False
        except OSError:
...
def configure_file_cache(
    max_size: int = 100, max_memory_mb: int = 50, default_ttl_seconds: int = 300
) -> FileCache:

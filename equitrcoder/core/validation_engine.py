...
class BaseValidator(IValidator[Any]):
    ...
    def validate(self, item: Any) -> Result[bool]:
        ...
        return Result(
            ...
            metadata={"warnings": warnings, "errors": errors, "validator": self.name},
        )
...
class FileValidator(BaseValidator):
    ...
    def validate_file_permissions(
        self, file_path: str, required_permissions: str = "r"
    ) -> ValidationResult:
        """
...
class ModelValidator(BaseValidator):
    ...
    def validate_model_config(self, model_config: Dict[str, Any]) -> ValidationResult:
        ...
        if errors:
            return ValidationResult(
                is_valid=False, errors=errors, validation_type=ValidationType.MODEL
            )
        ...
        if provider not in self.supported_providers:
            errors.append(
                f"Unsupported provider: {provider}. Supported: {', '.join(self.supported_providers)}"
            )
...
class APIValidator(BaseValidator):
    ...
    def validate_api_response(
        self, response: Dict[str, Any], expected_schema: Optional[Dict[str, Any]] = None
    ) -> ValidationResult:
        """
        ...
        if not isinstance(response, dict):
            ...
            return ValidationResult(
                is_valid=False, errors=errors, validation_type=ValidationType.API
            )
...
class ValidationEngine:
    ...
    def register_validator(
        self, validation_type: ValidationType, validator: BaseValidator
    ) -> None:
        """
        ...
        with self._lock:
            ...
            logger.debug(
                f"Registered validator {validator.name} for type {validation_type.value}"
            )
        ...
        for param, expected_type in param_types.items():
            if param in parameters and parameters[param] is not None:
                if not isinstance(parameters[param], expected_type):
                    errors.append(
                        f"Parameter '{param}' must be of type {expected_type.__name__}, got {type(parameters[param]).__name__}"
                    )
        ...
        with self._lock:
            ...
            if file_validators:
                ...
                if isinstance(validator, FileValidator):
                    return validator.validate_file_permissions(
                        file_path, required_permissions
                    )
    ...
    def validate_api_response(
        self, response: Dict[str, Any], schema_name: Optional[str] = None
    ) -> ValidationResult:
        """
        Validate API response
        ...
        return ValidationResult(
            is_valid=len(errors) == 0, errors=errors, validation_type=ValidationType.API
        )
        ...
        if not validation_result.is_valid:
            for error in validation_result.errors:
                if "missing required" in error.lower():
                    ...
                    guidance.append(
                        "Use a supported provider or add support for the new provider"
                    )
                else:
        ...
        with self._lock:
            ...
            for validation_type, validators in self.validators.items():
                info[validation_type.value] = [
                    {"name": validator.name, "rules_count": len(validator.rules)}
...
def validate_config(
    config: Dict[str, Any], schema_name: Optional[str] = None
) -> ValidationResult:
...
def validate_api_response(
    response: Dict[str, Any], schema_name: Optional[str] = None
) -> ValidationResult:
        ...
        def wrapper(*args, **kwargs):
            ...
            if not validation_result.is_valid:
                ...
                raise create_contextual_exception(
                    ...
                    recovery_suggestions=get_validation_engine().get_validation_guidance(
                        validation_result
                    ),
                )

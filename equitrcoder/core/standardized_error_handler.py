...
class StandardizedErrorHandler:
    ...
    def __init__(
        ...
        self._recovery_actions: Dict[ErrorCategory, List[RecoveryAction]] = {
            ErrorCategory.CONFIGURATION: [
                RecoveryAction(
                    "check_config", "Check configuration file syntax and values"
                ),
                RecoveryAction(
                    "reset_config", "Reset to default configuration", automated=True
                ),
                RecoveryAction(
            ...
            ErrorCategory.NETWORK: [
                RecoveryAction(
                    "retry_request", "Retry the network request", automated=True
                ),
            ...
            ErrorCategory.FILE_SYSTEM: [
                ...
                RecoveryAction(
                    "create_directory", "Create missing directories", automated=True
                ),
                RecoveryAction("check_disk_space", "Check available disk space"),
            ],
            ErrorCategory.MODEL: [
                RecoveryAction(
                    "validate_model", "Validate model name and availability"
                ),
                RecoveryAction(
                    "use_fallback_model", "Use fallback model", automated=True
                ),
    ...
    def handle_error(
        ...
        with self._lock:
            ...
            handled_error = HandledError(
                original_error=error, contextual_error=contextual_error, handled=True
            )
    ...
    def create_contextual_exception(
        self,
        message: str,
        category: ErrorCategory,
        ...
        contextual_error = ContextualError(
            ...
            or self._get_recovery_suggestions_by_category(category),
        )
    ...
    def _categorize_error(self, error: Exception) -> ErrorCategory:
        ...
        if any(
            ...
            for keyword in ["config", "setting", "parameter", "yaml", "json"]
        ):
        ...
        if any(
            keyword in error_msg
            for keyword in ["api key", "token", "auth", "credential", "unauthorized"]
        ):
        ...
        # Model errors
        if any(
            keyword in error_msg for keyword in ["model", "llm", "openai", "anthropic"]
        ):
    ...
    def _get_recovery_suggestions_by_category(
    ...
    def _escalate_error(self, error: ContextualError) -> EscalationResult:
        ...
        """
        escalation_level = (
            "high" if error.severity == ErrorSeverity.CRITICAL else "medium"
        )

        logger.critical(
            f"ESCALATED ERROR [{escalation_level.upper()}]: {error.message}",
            extra={"error_id": error.error_id, "escalation_level": escalation_level},
        )

        return EscalationResult(
            escalated=True, escalation_level=escalation_level, notification_sent=True
        )
...
def create_contextual_exception(
    ...
    handler = get_error_handler()
    return handler.create_contextual_exception(
        message, category, severity, context, recovery_suggestions
    )

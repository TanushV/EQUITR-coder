...
class PerformanceMonitor:
    ...
    def start_tracking(self, operation: str, metadata: Dict[str, Any] = None) -> TrackingContext:
    ...
    def record_llm_call(
        ...
        self.record_operation(
            ...
            metadata={
                ...
                "tokens_per_second": (prompt_tokens + completion_tokens) / execution_time if execution_time > 0 else 0
            }
        )
    ...
    def _check_cost_thresholds(self, new_cost: float):
        ...
        if new_cost > self.cost_threshold * 0.1:  # 10% of threshold for single operation
            alert = CostAlert(
    ...
    def get_performance_report(self, timeframe_hours: int = 24) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            # Operation breakdown
            operation_breakdown = defaultdict(lambda: {
                'count': 0,
                'avg_time': 0.0,
                'total_cost': 0.0,
                'success_rate': 0.0
            ...
            for metrics in recent_metrics:
                ...
                if op_stats['count'] == 1:
                    ...
                    op_stats['avg_time'] = (
                        (op_stats['avg_time'] * (op_stats['count'] - 1) + metrics.execution_time) 
                        / op_stats['count']
                    )
            ...
            return {
                ...
                "cost_efficiency": {
                    ...
                    "operations_per_dollar": total_operations / total_cost if total_cost > 0 else 0
                }
            }
    ...
    def get_cost_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                "threshold_usage_percent": (self._session_cost / self.cost_threshold * 100) if self.cost_threshold > 0 else 0,
                "remaining_budget": max(0, self.cost_threshold - self._session_cost),
                "session_tokens": self._session_tokens,
                "recent_alerts": len([a for a in self._cost_alerts if a.timestamp >= datetime.now() - timedelta(hours=1)])
            }
    ...
    def get_model_performance(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            llm_metrics = [m for m in self._metrics_history if m.operation == "llm_call"]
            ...
            for metrics in llm_metrics:
                ...
                if stats['calls'] == 1:
                    ...
                    stats['avg_response_time'] = (
                        (stats['avg_response_time'] * (stats['calls'] - 1) + metrics.execution_time)
                        / stats['calls']
                    )
                    
                    success_count = sum(
                        1 for m in llm_metrics

...
class MultiAgentOrchestrator:
    ...
    def __init__(
        ...
        self.context_manager = ContextManager(
            ...
            model=(
                getattr(supervisor_provider, "model", "gpt-4")
    ...
    def create_worker(
        self, config: WorkerConfig, provider: Optional[OpenRouterProvider] = None
    ) -> WorkerAgent:
    ...
    async def execute_task(
        ...
        try:
            ...
            if self.total_cost >= self.global_cost_limit:
                raise Exception(
                    f"Global cost limit ({self.global_cost_limit}) exceeded"
                )
    ...
    async def coordinate_workers(
        self, coordination_task: str, worker_tasks: List[Dict[str, Any]]
    ) -> Dict[str, Any]:

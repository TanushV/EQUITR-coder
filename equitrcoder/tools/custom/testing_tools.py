...
class RunTestsArgs(BaseModel):
    test_path: str = Field(
        default=".", description="Path to run tests from (default: current directory)"
    )
    test_pattern: str = Field(
        default="test_*.py", description="Test file pattern to match"
    )
...
class TestCoverageArgs(BaseModel):
    source_path: str = Field(
        default=".", description="Source code path to analyze coverage for"
    )
    test_path: str = Field(default="tests/", description="Test directory path")
    min_coverage: int = Field(
        default=80, description="Minimum coverage percentage required"
    )
...
class TestCoverage(Tool):
    ...
    async def run(self, **kwargs) -> ToolResult:
        try:
            ...
            return ToolResult(
                success=False, error="Coverage analysis timed out after 5 minutes"
            )
        except Exception as e:
            return ToolResult(success=False, error=str(e))
    ...
    def _generate_coverage_report(self, args):
        ...
        if report_result.returncode != 0:
            return ToolResult(
                success=False, error=f"Coverage report failed: {report_result.stderr}"
            )
        ...
        try:
            ...
            return ToolResult(
                ...
                error=(
                    f"Coverage {total_coverage:.1f}% is below minimum {args.min_coverage}%"
...
class LintCodeArgs(BaseModel):
    ...
    config_file: str = Field(
        default="", description="Path to linting config file (optional)"
    )
    ...
    async def run(self, **kwargs) -> ToolResult:
        try:
            ...
            return ToolResult(
                ...
                error=(
                    f"Linting found {result.stdout.count('error')} errors and {result.stdout.count('warning')} warnings"
                    if issues_found
                    else None
                ),
            )

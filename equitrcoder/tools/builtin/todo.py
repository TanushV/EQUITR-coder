...
class TodoManager:
    ...
    def _load_plan(self):
        ...
        if self.todo_file.exists() and self.todo_file.stat().st_size > 0:
            try:
                ...
                print(
                    f"Warning: Could not load or parse todo plan from {self.todo_file}: {e}"
                )
    ...
    def _save_plan(self):
        ...
        try:
            self.todo_file.write_text(
                self.plan.model_dump_json(indent=2), encoding="utf-8"
            )
    ...
    def _update_todo_in_group(
        self, group: TaskGroup, todo_id: str, status: str
    ) -> None:
    ...
    def get_next_runnable_groups(self) -> List[TaskGroup]:
        ...
        completed_group_ids = {
            g.group_id for g in self.plan.task_groups if g.status == "completed"
        }
        runnable_groups = []
        for group in self.plan.task_groups:
...
class ListTaskGroups(Tool):
    ...
    def get_description(self) -> str:
        return "Lists the high-level task groups, their specializations, dependencies, and statuses. This is the main way to see the overall project plan."

    def get_args_schema(self) -> Type[BaseModel]:
        return type("ListTaskGroupsArgs", (BaseModel,), {})
...
class ListAllTodos(Tool):
    ...
    def get_description(self) -> str:
        return "Lists ALL todos across ALL task groups with their statuses. Use this to see the complete project todo list."

    def get_args_schema(self) -> Type[BaseModel]:
        return type("ListAllTodosArgs", (BaseModel,), {})
...
class ListTodosInGroup(Tool):
    ...
    def get_name(self) -> str:
        return "list_todos_in_group"

    def get_description(self) -> str:
        return "Lists the detailed sub-tasks (todos) within a specific task group. Use this to see your work for your current assignment."
    ...
    async def run(self, **kwargs) -> ToolResult:
        ...
        if not group:
            return ToolResult(
                success=False, error=f"Task group '{args.group_id}' not found."
            )
...
class UpdateTodoStatus(Tool):
    ...
    def get_description(self) -> str:
        return "Updates the status of a specific todo item. Marking all todos in a group as 'completed' will automatically complete the entire group."
    ...
    async def run(self, **kwargs) -> ToolResult:
        ...
        return ToolResult(
            success=True,
            data=f"Todo '{args.todo_id}' updated. Parent group '{updated_group.group_id}' is now '{updated_group.status}'.",
        )
...
class BulkUpdateTodoStatus(Tool):
    class Args(BaseModel):
        ...
        status: str = Field(
            ...,
            description="The new status to apply to all todos, typically 'completed'.",
        )

    def get_name(self) -> str:
        return "bulk_update_todo_status"

    def get_description(self) -> str:
        return "Updates the status of multiple todo items at once. Efficient for marking several completed todos simultaneously."

...
async def test_clean_orchestrator_retries_on_invalid_json(monkeypatch, tmp_path):
    ...
    class FakeManager:
        def __init__(self):
            self.groups = {}
            self.todos = {}
        def create_task_group(self, group_id, specialization, description, dependencies):
    ...
    monkeypatch.setattr(todo_mod, "set_global_todo_file", lambda path: None, raising=False)
    ...
    async def fake_chat(messages=None, tools=None, **kwargs):
        ...
        return Resp(json.dumps([
            {"group_id": "grp1", "specialization": "default", "description": "", "dependencies": []}
        ]))

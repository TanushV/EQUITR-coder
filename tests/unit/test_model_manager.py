...
class TestModelManager:
    ...
    def test_get_provider_from_model(self):
        ...
        assert self.model_manager._get_provider_from_model("anthropic/claude-3-sonnet") == "anthropic"
        assert self.model_manager._get_provider_from_model("gpt-4") == "openai"
        assert self.model_manager._get_provider_from_model("claude-3-haiku") == "anthropic"
    ...
    def test_supports_function_calling(self):
        ...
        assert self.model_manager._supports_function_calling("openai/gpt-3.5-turbo") == True
        assert self.model_manager._supports_function_calling("anthropic/claude-3-sonnet") == True
    ...
    async def test_validate_model_no_function_calling(self):
        ...
        with patch.object(self.model_manager, '_supports_function_calling', return_value=False):
            with patch.object(self.model_manager, '_check_api_key_available', return_value=True):
    ...
    def test_format_model_error_no_function_calling(self):
        ...
        error_msg = self.model_manager.format_model_error("some-model", validation_result)
